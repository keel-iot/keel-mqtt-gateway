...
type FileProvider struct {
	...
	mu      sync.RWMutex
	byDevID map[string]*fileCredential // keyed by lower-case device UUID
...
func NewFileProviderWithTTL(path string, cacheTTL time.Duration) AuthProvider {
	return &FileProvider{
		path:      path,
		byDevID:   make(map[string]*fileCredential),
...
func (f *FileProvider) get(deviceID string) (*fileCredential, bool) {
	f.mu.RLock()
	c, ok := f.byDevID[deviceID]
	f.mu.RUnlock()
	return c, ok
}

// ValidatePassword checks the MQTT token against the bcrypt hash stored in the
// YAML file. Successful validations are cached for 30 seconds to reduce bcrypt
// load during reconnect storms.
func (f *FileProvider) ValidatePassword(_ context.Context, deviceID, token string) (*DeviceInfo, error) {
	if len(f.byDevID) == 0 {
	...
	if f.credCache.check(deviceID, token) {
		cred, ok := f.get(deviceID)
		if !ok {
			// Device disappeared since cache entry — should not happen in practice
			// since cache is invalidated on reload, but handle safely.
	...
	f.credCache.set(deviceID, token)
