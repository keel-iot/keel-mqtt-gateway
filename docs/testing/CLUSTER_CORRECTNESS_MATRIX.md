...
| Join/rejoin: kill all 3 Cores, restart, confirm gossip `rejoinIfIsolated` + `reconcileVoters` reconverge | (§1 rejoin) | **`internal/cluster/membership/rejoin_test.go`'s `TestRejoinIfIsolated_RepopulatesAfterRealDeathAndRestart`** — real `memberlist.Memberlist` instances on real loopback ports (not mocked), same-process multi-instance, exactly the scenario `membership.go`'s own comment calls out as a previously-found bug | Full multi-container (real separate binaries/processes) version still missing — the existing test proves the gossip logic, not the full container-restart path |
